...
- `/nas handoff-gate` is read-only and fail-closed; the local publication, security boundary review, and shared-vs-NAS scope split are recorded, but the architecture handoff remains blocked until a NAS repository plan and explicit remote-boundary approval are complete
