...
export const HELP_ENTRIES: HelpEntry[] = [
  ...
  },
  {
    name: "audit",
    category: "repo",
    usage: "/audit start check: <plans|lint|typecheck|tests|build|full> max_iterations:<1..3 optional> | /audit status | /audit review | /audit repair-plan | /audit stop | /audit repair | /audit repair-run | /audit repair-reviewed note:<optional> | /audit recheck | /audit repair-apply | /audit repair-revert | /audit repair-cleanup",
    short: "Fix audit checkeket futtat, review/repair-plan osszegzest ad, repair approvalt ker, izolalt repair turnt indithat, recheckel, es kulon jovahagyassal apply/revert source handoffot kezelhet.",
    details: [
      ...
      "`/audit repair-plan` read-only previewt ad a kesobbi izolalt Codex repair contractrol: cel-check, bizonyitek, workspace allapot, role phases, scope, tiltott muveletek.",
      ...
      "Ugyanarra az audit iterationre masodik started repair-run nincs; elobb review, majd `/audit recheck` kell. Sikertelen, nem stagnalt recheck utan a kovetkezo iterationben uj explicit repair-run indithato, ha maradt budget.",
  ...
  },
  {
    name: "nas",
    category: "ops",
    usage: "/nas status | /nas doctor | /nas worker-status | /nas worker-deploy-verify | /nas handoff-gate | /nas deploy-status | /nas deploy-plan | /nas deploy-apply | /nas rollback-plan commit:<optional> | /nas rollback-apply commit:<sha> | /nas container-status | /nas request check:<plans|lint|typecheck|tests|build|full> | /nas requests status:<all|queued|completed|failed> limit:<1-10> | /nas request-status request:<id-prefix> | /nas mailbox box:<inbox|outbox|archive> limit:<1-10> | /nas mailbox-status | /nas results limit:<1-10> | /nas bridge action:<status|start|stop|restart> | /nas smoke | /nas sync-status",
    ...
    details: [
      ...
      "`/nas rollback-plan commit:<optional>` read-only preview: megmutatja a jelenlegi deploy azonossagot, ellenorizheti a valasztott Git commitot, es hogy rollback apply meg tiltott.",
      "`/nas rollback-apply commit:<sha>` approval-gated BotOps `nas.rollback.apply` jobot hoz letre; a worker csak approval utan futtathatja a fix rollback helpert es utana kotelezo deploy verifiert.",
  ...
  },
  {
    name: "ops",
    category: "ops",
    usage: "/ops status | /ops workers | /ops jobs limit:<1-25> | /ops preview job_id:<id> | /ops approve job_id:<id> | /ops cancel job_id:<id> | /ops recover job_id:<id> | /ops logs job_id:<id>",
    ...
    details: [
      ...
      "`/ops preview` read-only modon megmutatja, pontosan milyen muveletre es validaciora szolna a jovahagyas.",
  ...
  },
  {
    ...
    details: [
      ...
      "`commit`, `push` es `restart` request utan a valasz kiirja az approval scope-ot, a tiltott akciokat, es a kovetkezo `/ops preview` -> approve/cancel lepest.",
