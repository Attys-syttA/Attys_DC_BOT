...
import { execute, renderMappingFields } from "./mappings.js";
...
describe("/mappings", () => {
  ...
  it("lists mapping totals and duplicate groups", async () => {
    ...
    expect(embed.data.fields[0].value).toContain("<#current-channel>");
    expect(embed.data.fields[0].value).toContain("<#legacy-forum>");
  });
