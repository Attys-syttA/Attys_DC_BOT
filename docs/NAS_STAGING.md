...
After rebuild, the helper first checks the NAS container status through the restricted SSH status wrapper. If the container already reports the current commit-specific image tag, the helper skips the long same-process SMB snapshot polling and goes directly to a shorter final verifier retry path. Intermediate verifier retries use JSON/silent mode and print only short not-ready summaries with the failed check names; the full checklist is still printed by the final successful or diagnostic verifier. The full deploy verifier still decides success; the image tag is only an early signal that the rebuild reached the NAS runtime.
