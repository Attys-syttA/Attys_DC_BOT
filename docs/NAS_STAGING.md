...
After rebuild, the default snapshot polling timeout is 180 seconds. This gives the NAS control-plane loop and SMB share visibility enough time to publish the new `logs\nas-control-plane-status.json` snapshot before the final verifier runs. Override with `-WaitAfterRebuildSec <seconds>` only after an explicit operator decision.
