...
- NAS deploy-plan Discord preview: a read-only `/nas deploy-plan` a meglévo `nas:deploy` helper dry-run elonezetet mutatja public-safe modon, `-Apply` kapcsolo, NAS share iras, container rebuild es restart nelkul.
- NAS deploy-plan rebuild expectation: a preview most kulon `will-rebuild=no|yes|unknown` sort mutat a jelenlegi deploy verifier allapota alapjan, hogy approval elott latszodjon a varhato rebuild kockazat.
- NAS deploy-apply BotOps gate: a `/nas deploy-apply` approval-gated `nas.deploy.apply` jobot hoz letre, amelyet csak a NAS worker vehet fel explicit approval utan. A worker csak a fixed `nas:deploy -- -Apply` helpert futtathatja, utana kotelezo `nas:deploy:verify` post-checkkel; Discord parancsbol nincs kozvetlen deploy.
- BotOps service restart post-check: a Windows worker `service.restart` helper csak `win-start.bat` siker es utana zold `doctor:local` mellett zár `Completed` allapotban.
- NAS rollback preview: a `/nas rollback-plan` csak read-only rollback tervezesi nezetet ad, jelenlegi deploy azonossaggal es tiltott rollback apply allapottal; rollback vegrehajtas nincs.
...
- Audit repair contract preview: `/audit repair-plan` read-only szerződés-előnézetet ad a későbbi izolált Codex repairhez, strukturált `audit-repair-contract/v2` contractból. A nézet public-safe módon mutatja a cél checket, legutóbbi bizonyítékot, repair workspace állapotot, planner/executor/validator role phase sorrendet, scope-ot, kötelező validációt, prompt readiness állapotot és tiltott műveleteket; tényleges Codex repair turnt továbbra sem indít.
...
- NAS deploy verifier retry output: a `nas:deploy -- -Apply` köztes verifier retryjai rövid not-ready sort írnak, a teljes checklist csak a végső sikeres/diagnosztikai verifiernél jelenik meg.
- Audit repair-reviewed note: `/audit repair-reviewed` opcionális public-safe `note` értéket is rögzíthet a repair execution ledgerben, továbbra is check/file/Git/deploy művelet nélkül.
- Audit repair-cleanup command: `/audit repair-cleanup` terminál audit jobnál non-force `git worktree remove` útvonalon próbálja eltávolítani a matching izolált repair worktree-t; dirty/unsafe esetben `cleanup_failed` marad.
- Audit cleanup next-action guidance: `/audit review` terminál job és cleanup-ready repair workspace mellett `/audit repair-cleanup` következő lépést mutat.
- Audit removed workspace visibility: removed repair workspace esetén az audit status/review/repair-plan `removed` change summaryt mutat és nem próbál törölt lokális pathot újraolvasni.
- Audit repair-plan prompt readiness hardening: `/audit repair-plan` csak prepared/retained repair workspace és nem-passed audit evidence mellett mutat `ready` prompt állapotot; hiányos előfeltételnél read-only `blocked` előnézetet ad.
- Audit review budget guidance: `/audit review` explicit iteration budget sort mutat, és kimerült budget mellett nem ajánl új repair/recheck lépést.
- Audit start bounded max-iterations option: `/audit start` opcionális `max_iterations` opcióval 1-3 közötti recheck budgetet állíthat; default 2 és runtime hard maximum 3.
- Audit repair-plan blocked issue visibility: `/audit repair-plan` public-safe `repair prompt issues` sort ad a blocked readiness okáról.
- Audit repair flow synthetic acceptance: `src/audit/repair-flow-acceptance.ts`, `src/audit/repair-flow-acceptance.test.ts` és `npm run audit:repair-flow:smoke` synthetic Git repo és izolált repair worktree mellett bizonyítja az első failed evidence -> guardolt repair execution ledger -> manual review marker -> isolated recheck -> source worktree változatlanság -> dirty cleanup fail-closed retention -> explicit tisztítás utáni non-force cleanup láncot; a smoke a standard `npm run check` kapu része.
- Audit job/step contract versioning: `src/audit/job-step-contract.ts` rögzíti az `audit-job-step-contract/v1` public-safe átadási contractot a persistált audit job/step rekordokból. A synthetic repair-flow smoke a job/step contract és a repair contract verzióját is kiírja.
- Audit repair-run live Discord smoke: operatori `_smoke` synthetic projekten a Discord parancsútvonal végigment. A `tests` check először `failed exit=1` eredményt adott, `/audit repair` izolált worktree-t készített, `/audit repair-plan` `repair prompt: ready` állapotot mutatott, `/audit repair-run` tracked repair executiont indított, `/audit repair-reviewed` review note-ot rögzített, majd `/audit recheck` az izolált worktree-ben `tests passed exit=0` eredménnyel `completed` állapotba vitte a jobot.
- Audit repair-cleanup live fail-closed smoke: dirty izolált repair worktree mellett a `/audit repair-cleanup` non-force cleanup útvonal megtagadta az eltávolítást, `cleanup_failed` státuszt rögzített, és a worktree-t megtartotta manual review-ra. A normál `_smoke` source worktree tiszta maradt.
- Audit repair-apply handoff: külön default-off `/audit repair-apply` parancs és `DISCORD_ENABLE_AUDIT_REPAIR_APPLY=false` flag készült. Csak completed job, reviewed repair execution, passing isolated recheck, tiszta source worktree, matching source/repair `HEAD`, és egyszerű tracked repair módosítások mellett visz át patch-et a normál source worktree-be, majd ott újrafuttatja az eredeti named checket; commit/push/deploy/branch merge/cleanup továbbra sincs.
- Audit repair-apply live smoke: operatori `_smoke` synthetic jobon a `/audit repair-apply` sikeresen átvette az izolált repair módosítást a normál source worktree-be. Lokális ellenőrzés szerint a source `M check.js`, a step history `tests failed` -> isolated `tests passed` -> source `tests passed`, a repair workspace státusz `applied`; commit/push/deploy/branch merge/cleanup nem történt.
- Applied repair cleanup hardening: `/audit repair-cleanup` `applied` workspace esetén csak akkor takarítja az izolált repair worktree-t, ha a source és repair `HEAD` egyezik, a repair oldalon nincs staged/untracked/rename/delete változás, és a source/repair diff azonos. A source worktree módosításait nem állítja vissza.
- Applied cleanup live smoke: operatori `_smoke` synthetic jobon a `/audit repair-cleanup` az `applied` workspace-t `removed` állapotba vitte. Lokális ellenőrzés szerint az izolált repair worktree eltűnt a `git worktree list` kimenetből, a DB státusz `removed`, és a normál source worktree szándékosan dirty `M check.js` maradt.
- Audit source handoff visibility: a jövőbeli post-apply cleanup `applied_removed` státuszt rögzít, és `/audit review` cleanup után is mutatja, hogy az átvett javítás már a normál source worktree-ben van; commit/revert továbbra is `/audit`-on kívüli operatori döntés.
- Audit repair-revert handoff: `/audit repair-revert` külön source-write flag alatt, csak `applied` státuszú, még létező izolált repair worktree mellett tudja visszafordítani az átvett patch-et a normál source worktree-ből, source-side named check validációval és commit/push/deploy/cleanup nélkül. A `reverted` cleanup tiszta source mellett `reverted_removed` állapotba takarít.
- Audit repair contract role phases: `audit-repair-contract/v2` explicit planner/executor/validator role phase listát ad a promptnak és a `/audit repair-plan` outputnak, egyelőre egyetlen izolált repair turnön belüli felelősségi szerződésként.
...
- Live BotOps NAS deploy verifier smoke: synthetic approved `nas.deploy.verify` jobot a live NAS worker completed állapotban zárt `verifier passed` eredménnyel; queue utána üres, worker error logok üresek.
...
- Repair eredményének utókezelése: apply után a normál source worktree szándékosan dirty marad, ezért staging vagy visszaállítás továbbra is külön emberi/operátori döntés. A commit és push approval-gated worker helper elkészült, de commit csak már staged változásból, push pedig csak már commitolt, clean és not-behind upstream állapotból indul.
...
- **részben kész:** `/audit repair-reviewed` jelöli, hogy az operátor kézzel átnézte az elindított repair executiont, és opcionális public-safe review note-ot is rögzíthet; a validálás továbbra is külön `/audit recheck`.
...
- **részben kész:** `/audit repair-cleanup` terminál job után non-force módon el tudja távolítani a matching izolált repair worktree-t, dirty/unsafe esetben pedig `cleanup_failed` státuszban megőrzi az állapotot.
- **részben kész:** `/audit repair-apply` külön source-write approval gate mögött, sikeres reviewed isolated recheck után tudja patchként átvenni az egyszerű tracked javítást a normál source worktree-be, source-side validációval és automatikus commit/push/deploy/cleanup nélkül.
- **kész live smoke szinten:** `/audit repair-apply` operatori `_smoke` synthetic jobon sikeresen source-side `tests passed` validációt adott, és a repair workspace `applied` státuszba került; a source worktree szándékosan dirty maradt.
- **részben kész:** `/audit repair-cleanup` applied workspace mellett diff-egyezéses guard után tudja eltávolítani az izolált repair worktree-t, miközben a source dirty handoff állapotot érintetlenül hagyja.
- **kész live smoke szinten:** `/audit repair-cleanup` az operatori `_smoke` applied job után eltávolította az izolált repair worktree-t, a DB státusz `removed` lett, és a normál source dirty handoff állapot változatlan maradt.
- **részben kész:** post-apply cleanup most `applied_removed` státuszt használ, ezért `/audit review` a törölt izolált worktree után is megőrzi a source handoff commit/revert döntési jelzést.
- **részben kész:** `/audit repair-revert` a még meglévő `applied` worktree diffjéből guardoltan vissza tudja fordítani a source handoffot, majd `reverted` állapotból tiszta source mellett cleanupolható.
- **részben kész:** `/audit review` terminál jobnál már cleanup next-actiont mutat, ha van még eltávolítható izolált repair workspace.
- **részben kész:** removed repair workspace esetén a status/review/repair-plan kimenet nem olvassa újra a törölt worktree pathot, hanem `removed` összegzést ad.
- **részben kész:** `/audit repair-plan` prompt readiness fail-closed: nincs `ready`, ha nincs prepared/retained repair workspace vagy nincs nem-passed audit evidence.
- **részben kész:** `/audit review` kimerült iteration budgetnél már nem javasol új repair/recheck lépést.
- **részben kész:** `/audit start max_iterations` opcionálisan 1-3 közötti bounded budgetet enged, default 2 mellett.
- **részben kész:** `/audit repair-plan` blocked prompt readiness esetén public-safe okot is megjelenít.
- **részben kész:** azonos public-safe failed fingerprint esetén a recheck `stagnated` állapotban megáll, és a recheck nem lépi túl a job iteration budgetjét; sikertelen, nem stagnált recheck után a következő iteration kézi `/audit repair-plan` -> `/audit repair-run` -> `/audit recheck` útvonala már látható a review guidance-ben.
- **kész automatizált acceptance szinten:** synthetic Git repo + izolált repair worktree tesztben és `npm run audit:repair-flow:smoke` parancsban egy approved repair execution injektált Codex starterrel javít, kézi review marker után ugyanaz a named check átmegy rechecken, a normál source worktree változatlan marad, dirty cleanup fail-closed módon retained állapotban marad, majd explicit synthetic operátori tisztítás után a non-force cleanup eltávolítja a repair worktree-t.
...
- **részben kész:** review guidance kimerült budgetnél fail-closed next-actiont ad, csak ledger-only `/audit repair-reviewed` marad engedett, ha már started repair execution van, és megmaradt budget + prepared/retained workspace mellett explicit repair-plan/repair-run/recheck retry útvonalat mutat.
- **részben kész:** `/audit start` schema és runtime guard a default 2 / hard max 3 iteration budgetet érvényesíti.
...
- **részben kész:** determinisztikus, read-only `/audit repair-plan` contract renderer, strukturált `audit-repair-contract/v2` validátor és prompt readiness check elkészült. A contract explicit planner/executor/validator role phase listát tartalmaz; ez még nem külön multi-agent futtatás, hanem egy Codex repair turnön belüli public-safe felelősségi szerződés.
...
- **kész automatizált acceptance szinten:** a repair execution prompt/contract csak valid prepared worktree + non-passed evidence mellett indul, és a teszt az executor írását kizárólag az izolált worktree-re korlátozza.

### Szelet 7 — NAS handoff gate

Ez a szelet az egységes `Attys_DC_BOT` source-of-truth irányhoz igazodik; az `Attys_DC_BOT_NAS` repo történeti/reference állapotú, nem aktív NAS runtime cél.
...
- az `Attys_DC_BOT` saját `AGENTS.md`, `docs/STATE.md` és aktív terve írja le a tényleges NAS/BotOps irányt; az `Attys_DC_BOT_NAS` csak referencia/superseded anyagként használható;
- a user külön jóváhagyta a remote/multi-machine architecture boundary megváltoztatását.
- **kész jelen checkpoint szinten:** read-only `/nas handoff-gate` parancs készült a fenti előfeltételek public-safe láthatóságára. A local publication, security boundary review, shared-vs-NAS scope split, egységes NAS/BotOps plan és explicit remote-boundary approval `OK`, ezért a gate `ready`. A gate továbbra sem ír NAS source/share adatot, nem indít remote executiont, nem deployol, nem rebuildel és nem restartol; ezek külön command-by-command approval gate-ek maradnak.
