...
- Audit repair execution tracking: additive SQLite ledger és belső tracked-start helper készült a későbbi Codex repair turnhoz. A `/audit status` és `/audit review` public-safe módon mutatja a repair execution státuszt rövid thread/turn prefixekkel és sanitizált summaryval; tényleges Discord repair-run parancs még nincs.
