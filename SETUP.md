...
- `/audit repair-run` is additionally disabled unless `DISCORD_ENABLE_AUDIT_REPAIR_EXECUTION=true` is set in `.env`; when enabled it can start one tracked Codex repair turn in the prepared or retained isolated repair worktree only when there is non-passed audit evidence, remaining iteration budget, and no already-started repair execution for the same iteration. After a failed non-stagnated recheck advances the iteration, `/audit review` shows the guarded retry commands. It does not merge, commit, push, deploy, or write the normal source worktree.
...
- `/audit repair-revert` uses the same source-write flag and can revert only an `applied` handoff while the isolated repair worktree still exists and exactly matches the source diff. After cleanup records `applied_removed`, `/audit` cannot safely revert because the isolated patch source is gone.
- `/audit repair-cleanup` is available only after the audit job is terminal. It removes only the matching isolated repair worktree with non-force `git worktree remove`; after `/audit repair-apply`, it first verifies that the repair diff already matches the source diff, then clears only the isolated worktree before removal and records the handoff as `applied_removed` for later review. If Git refuses because the workspace is dirty or otherwise unsafe, the workspace remains and the record becomes `cleanup_failed`.
- `/nas handoff-gate` is read-only and fail-closed. It now treats the local source publication, security boundary review, and shared-vs-NAS scope split as recorded, while still blocking NAS repo source writes and remote execution changes until a dedicated NAS repository plan and explicit remote-boundary approval exist.
